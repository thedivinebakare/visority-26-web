...
try{
  ...
  const posts=[];let fail=false;await page.setRequestInterception(true);page.on('request',req=>{if(req.url().endsWith('/api/register')){posts.push({url:req.url(),body:req.postData()});if(fail)req.respond({status:503,contentType:'application/json',body:'{"saved":false,"message":"Mock failure"}'});else req.respond({status:200,contentType:'application/json',body:'{"saved":true}'});}else req.continue();});
  async function complete(pass){
    ...
    const before=posts.length;await page.click('#reg-submit');await pause(500);assert.equal(posts.length,before+1,'One POST per submission');return JSON.parse(posts.at(-1).body);
  }
