...
try{
 ...
 for(const [name,width,height] of [['desktop',1440,1000],['laptop',1280,800],['mobile',390,844],['small',360,800],['tablet',768,1024]]){
  await page.setViewport({width,height});await page.goto('http://localhost:3000',{waitUntil:'networkidle0'});await page.evaluate(async()=>{document.documentElement.style.scrollBehavior='auto';document.querySelectorAll('img').forEach(i=>i.loading='eager');await document.fonts.ready;await Promise.all([...document.images].map(i=>i.decode().catch(()=>{})));});
  ...
  for(const [label,selector] of [['ambassadors','#ambassador-program'],['special-guest','.speaker-ikenna'],['speaker','.speaker-mandy'],['pricing','#pricing'],['schedule','#schedule'],['chapter','.chapter'],['problem','#experience']]){
 ...
 await page.setViewport({width:1440,height:1000});await page.goto('http://localhost:3000');await page.evaluate(()=>document.documentElement.style.scrollBehavior='auto');await page.$eval('#experience',e=>e.scrollIntoView());await delay(250);assert.equal(await page.$eval('.ticket-dock',e=>e.hidden),false);await page.$eval('#pricing',e=>e.scrollIntoView());await delay(250);assert.equal(await page.$eval('.ticket-dock',e=>e.hidden),true);
 await page.emulateMediaFeatures([{name:'prefers-reduced-motion',value:'reduce'}]);await page.goto('http://localhost:3000');assert.equal(await page.$eval('.marquee-track',e=>getComputedStyle(e).animationName),'none');assert.equal(await page.$eval('[data-reveal]',e=>getComputedStyle(e).opacity),'1');
 await page.setJavaScriptEnabled(false);await page.goto('http://localhost:3000');assert.equal(await page.$eval('[data-reveal]',e=>getComputedStyle(e).opacity),'1');await page.setJavaScriptEnabled(true);
 await page.goto('http://localhost:3000');await page.evaluate(()=>{document.documentElement.style.scrollBehavior='auto';const original=URL.createObjectURL;URL.createObjectURL=function(blob){window.calendarContent=blob.text();return original.call(this,blob);};});
